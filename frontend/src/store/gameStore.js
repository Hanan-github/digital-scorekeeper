  ...
  persist(
    (set, get) => ({
      ...
      completeConfiguration: () => {
        const state = get();
        const homeStats = state.homeTeam.players.map((p) =>
          createEmptyPlayerStats(p.name)
        );
        const awayStats = state.awayTeam.players.map((p) =>
          createEmptyPlayerStats(p.name)
        );
      ...
      addPoints: (team, playerIndex, points, type) => {
        ...
        const statsKey =
          team === "home" ? "homePlayerStats" : "awayPlayerStats";
        ...
        set((state) => {
          ...
          // Update play by play
          const teamName =
            team === "home" ? state.homeTeam.name : state.awayTeam.name;
